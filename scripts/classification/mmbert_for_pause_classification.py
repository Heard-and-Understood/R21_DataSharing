"""
Multi-Modal BERT Baseline for Pause Classification. Classifies pause types using pre-pause text 
transcriptions and audio spectrograms. Based on the work of Kiela et al. (2020) w/ MMBT.
...
NOTE: regarding the "freezing" optional run parameters...

For pause classification with ~6,000 samples:

1. Start with both frozen for a quick baseline:
   --freeze_bert --freeze_resnet

2. Then try fine-tuning BERT only (freeze ResNet):
   --freeze_resnet

3. If you have time/resources, try fine-tuning both:
   (no freeze flags)
...
from transformers import BertTokenizer, BertModel, Subset

parser = argparse.ArgumentParser(
    description='Multi-Modal BERT Baseline for Pause Classification'
)

# data args
# NOTE: these should be generated by prep_mmbert_input.py with --create_split
parser.add_argument(
    '--data_dir',
    type=str,
    required=True,
...
parser.add_argument(
    ...
    type=float,
    default=2e-05,
    help='Learning rate for optimizer (default: 2e-05)'
)
parser.add_argument(
    '--pos_class_threshold',
    type=float,
...
parser.add_argument(
    '--save_model',
    action='store_true',
    help='Save the best model checkpoint to disk (full model with BERT + ResNet + head). By default, no model is saved.'
)
...
parser.add_argument(
    '--output_dir',
    type=str,
    default='./outputs/mmbert_baseline',
...
MAX_TEXT_LENGTH = 512    # BERT's maximum sequence length
IMAGE_SIZE = 224         # standard size for ResNet
BATCH_SIZE = 8           # adjust based on your GPU memory (8 for ARM Mac/MPS, 16+ for CUDA)
NUM_EPOCHS = 20
# LEARNING_RATE = 2e-05     # started at 2e-5, tried 5e-6, manual best at 1e-5
WARMUP_RATIO = 0.1
WEIGHT_DECAY = 0.1      # started at 0.01, try between 0.05 - 0.1
EARLY_STOPPING_PATIENCE = 3   # when to stop once loss stops changing
...
class PauseDataset(Dataset):
    ...
    def __init__(
        ...
        Parameters:
            ...
            image_transform (Optional[transforms.Compose]) - transform to apply to spectrogram images
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        # print(f'Now attempting to load image at: {image_path}')
        try:
...
class MultiModalBERT(nn.Module):
    """
    Multi-modal BERT model for pause classification.

    NOTE: based on MMBT from Kiela et al. (2020)
    """
    ...
    def __init__(
        ...
        bert_hidden_size = self.bert.config.hidden_size   # 768 dimensions for bert-base
        ...
        resnet_hidden_size = 2048  # ResNet50 final layer size
        ...
        self.image_projection = nn.Linear(resnet_hidden_size, 512)
        
        # cross-modal fusion layer
        self.fusion = nn.Sequential(
            nn.Linear(1024, 512),  # 512 (text) + 512 (image)
            nn.ReLU(),
            nn.Dropout(dropout),
            nn.Linear(512, 256),
            nn.ReLU(),
    ...
    def forward(
        ...
        Parameters:
            ...
            image: Spectrogram images [batch_size, 3, 224, 224]
        ...
        text_features = bert_output.last_hidden_state[:, 0, :]  # [batch_size, 768]
        ...
        image_features = image_features.squeeze(-1).squeeze(-1)  # [batch_size, 2048]
        image_features = self.image_projection(image_features)  # [batch_size, 512]
        image_features = self.image_dropout(image_features)
        
        # concatenate modalities
        fused_features = torch.cat([text_features, image_features], dim=1)  # [batch_size, 1024]
...
def seed_everything():
    ...
    https://discuss.pytorch.org/t/cannot-reproduce-bert-training-results-despite-following-all-reproducibility-guideness/175328 
    """
    # diagnostic output for CUDA/PyTorch setup
    ...
    np.random.seed(RANDOM_SEED)              # numpy seed
    random.seed(RANDOM_SEED)                 # python random module seed
    torch.backends.cudnn.benchmark = False
    torch.backends.cudnn.deterministic = True
    # use relaxed deterministic mode (allows minor non-determinism in CUDA operations)
    # avoids runtime errors with CUDA operations that don't support full determinism
...
def set_device(specified_device):
    ...
    NOTE: CUDA should already be initialized by seed_everything() before this function
    is called, which ensures proper detection in Slurm/GPU environments.
    """
    available = True
    if not specified_device or specified_device == 'cpu':
        ...
        if specified_device == 'cuda':
            # check availability (CUDA should already be initialized by seed_everything)
        ...
        if not available:
            ### TODO: TEMPORARY: abort if no CUDA available
...
def train_epoch(
    ...
    # disable progress bar when stdout/stderr redirected (e.g. to log file) for readable logs
    ...
    for batch in progress_bar:
        ...
        # TODO: try this instead for predicted
...
def evaluate(
    ...
    with torch.no_grad():
        for batch in progress_bar:
            ...
            # TODO ..trying something else for prediction threshold to help predict positive class
            probs = torch.softmax(logits, dim=1)
            predicted = (probs[:, 1] >= pos_class_threshold).long()  # try 0.2, 0.15, 0.1
...
def create_downsampled_dataset(
    ...
    This is useful when class imbalance is very large and you prefer to
    balance by removing majority-class examples instead of using class weights.
    ...
    Returns:
        Subset - a PyTorch Subset containing a class-balanced sample with proportional participant representation
    """    
...
def create_quick_test_subset(train_dataset, idx_to_label, random_seed, samples_per_class=50):
    ...
    Randomly samples equal numbers from each class (default: 50 per class) to create
    a balanced training subset. This is useful for local testing with limited resources.
    
    Parameters:
        train_dataset - full training dataset (PauseDataset instance)
        idx_to_label - dictionary mapping label indices to label names
        random_seed - random seed for reproducibility
        samples_per_class - number of samples to sample from each class (default: 50)
    
    Returns:
        Subset: A PyTorch Subset containing the balanced sample
    """
    print("\nQUICK TEST MODE: creating balanced subset")
    
    # get all labels and group indices by class
    class_indices = {}
    for idx in range(len(train_dataset)):
        sample = train_dataset[idx]
        label = sample['label'].item()
    ...
    # randomly sample from each class (or all available if less than samples_per_class)
    subset_indices = []
...
def estimate_checkpoint_size_mb(
    ...
    if save_model:
        ...
        model_bytes = sum(
            ...
            if not k.startswith('bert.') and not k.startswith('resnet.')
        )
    ...
    optimizer_bytes = 2 * optimizer_param_count * 4  # float32

    # metadata (epoch, scalars, label_to_idx, etc.) ~negligible; zip overhead ~5%
    total = (model_bytes + optimizer_bytes) * 1.05 + 50 * 1024  # +50KB for metadata
    return total / (1024 * 1024)

def check_output_dir_writable(output_dir: Path) -> None:
    """
    Verify that we can create and write a file in output_dir.
    Raises RuntimeError with a clear message if not (permissions, quota, etc.).
    """
...
def run_mmbert_pipeline(
    ...
    """
    print("\nBeginning training of Multi-Modal BERT for pause classification")
    ...
    print(f"  Freeze ResNet:          {freeze_resnet}")
    ...
    if save_model:
        print("   ...best model will be saved to disk (full model: BERT + ResNet + head)")
    ...
    if hybrid_balance and not quick_test:
        ...
        # standard downsampling: 1:1 ratio (match minority class)
    ...
    # convert class names to strings (in case labels are numeric)
    ...
    model = MultiModalBERT(
        ...
        dropout=dropout   # defaults to 0.3; we should increase for smaller datasets
    )
    ...
    if use_class_weights or hybrid_balance:
        ...
        # check if custom class weights are provided by user
    ...
    # add prediction column (predictions are in same order as CSV rows)
    val_df['mmbert_predicted_label'] = val_pred_labels
    test_df['mmbert_predicted_label'] = test_pred_labels
    
    # save prediction CSVs with prefixes
...
if __name__ == '__main__':
    ...
    # pattern: [label_name]_feature_data_[dataset_type].csv
    ...
    label_col = args.label_name  # label column name matches the label_name used in filenames
